...
<section class="hero"><h1>Find what blocks agents from buying your API.</h1><p>Enter one public endpoint. CAPI2 returns a free, evidence-based preview. Upgrade only when the findings are useful.</p><div class="flow">Public URL → free preview → visible gaps → $29 full report → optional implementation</div></section>
<section class="preview"><h2>Run the free commerce preview</h2><p>No wallet, account or sales call. Public HTTPS endpoints only.</p><form id="previewForm"><input id="target" type="url" required placeholder="https://your-api.example"><button type="submit">Check my endpoint →</button></form><div class="result" id="result"></div></section>
<section class="grid"><article class="card"><b>$29 · Assurance Report</b><p>Security, discovery and agent-commerce readiness combined in one inline report.</p><a class="ghost" href="/docs#/reports/commerce_assurance_v1_reports_commerce_assurance_post">Buy via x402</a></article><article class="card"><b>$49 · Endpoint Check</b><p>Readiness evidence and prioritized implementation fixes.</p><a class="ghost" href="/docs#/sales/endpoint_check_v1_products_endpoint_check_post">Buy via x402</a></article><article class="card"><b>$149 · Launch Pack</b><p>Positioning, offer ladder, discovery plan and launch checklist.</p><a class="ghost" href="/docs#/sales/launch_pack_v1_products_launch_pack_post">Buy via x402</a></article><article class="card"><b>$199 · Integration</b><p>Policy, receipts and acceptance tests for production.</p><a class="ghost" href="/docs#/sales/verification_integration_v1_products_verification_integration_post">Buy via x402</a></article></section>
<script>document.getElementById('previewForm').addEventListener('submit',async function(e){e.preventDefault();var out=document.getElementById('result'),url=document.getElementById('target').value;out.style.display='block';out.textContent='Checking public discovery surfaces…';try{var r=await fetch('/v1/sales/preflight',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({target_url:url})});var d=await r.json(),checks=d.audit&&d.audit.checks||[],missing=checks.filter(function(x){return !x.present}).map(function(x){return x.name});out.textContent='Readiness score: '+(d.audit&&d.audit.readiness_score||0)+'/100\nRecommended next step: '+(d.recommended_product&&d.recommended_product.name||'Commerce Assurance Report')+'\n\nMissing or unreachable: '+(missing.join(', ')||'none detected')+'\n\nFull security + discovery report: $29 via x402.'}catch(err){out.textContent='The preview could not complete. Confirm this is a public HTTPS URL and retry.'}})</script>
