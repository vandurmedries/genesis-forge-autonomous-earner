...
<title>CAPI2 — Website & API Risk Report</title>
<meta name="description" content="Get an instant website and API risk report covering security headers, TLS, agent discovery and x402 readiness.">
<style>
...
async def public_story():
    """Public, evidence-first company story suitable for partners and directories."""
    return """<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>CAPI2 — Verifiable commerce for autonomous AI agents</title>
<meta name="description" content="How CAPI2 gives autonomous AI agents a controlled way to discover, purchase and verify digital services with x402.">
<style>body{margin:0;background:#07130e;color:#eef7f2;font:17px/1.65 system-ui,sans-serif}.w{max-width:850px;margin:auto;padding:30px}nav{display:flex;justify-content:space-between;align-items:center}.pill{background:#bdf45b;color:#102117;padding:6px 11px;border-radius:999px;font-size:13px;font-weight:800}header{padding:75px 0 35px}h1{font-size:clamp(45px,8vw,78px);line-height:.98;letter-spacing:-.05em;margin:0 0 22px}h2{margin-top:48px;font-size:30px}p,li{color:#b9cbc2}.lead{font-size:22px}.proof{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:35px 0}.card{background:#0e2219;border:1px solid #2b493b;border-radius:14px;padding:18px}.card b{display:block;color:#bdf45b;font-size:24px}.flow{padding:20px;border-left:4px solid #bdf45b;background:#0e2219;font-weight:750}.cta{display:inline-block;margin:22px 0;background:#bdf45b;color:#102117;padding:13px 18px;border-radius:10px;text-decoration:none;font-weight:850}.note{font-size:14px;color:#8fa69b;border-top:1px solid #294539;margin-top:55px;padding-top:22px}@media(max-width:650px){.proof{grid-template-columns:1fr}header{padding-top:50px}}</style></head>
<body><main class="w"><nav><b>CAPI2</b><span class="pill">Public company story</span></nav><header><h1>AI agents can buy services. CAPI2 makes the transaction verifiable.</h1><p class="lead">CAPI2 is an agent-commerce infrastructure project from Belgium. It gives autonomous agents machine-readable discovery, bounded payments, immediate digital delivery and evidence receipts—without requiring an account or subscription.</p><a class="cta" href="/buy">Run the free risk preview →</a></header>
<section class="proof"><div class="card"><b>12</b>discoverable paid x402 resources</div><div class="card"><b>$0.01</b>entry price for repeat-use utilities</div><div class="card"><b>$29</b>instant Website &amp; API Risk Report</div></section>
<h2>The problem</h2><p>AI agents can call APIs, but commercial use still breaks at the boundaries. The agent may not know what an endpoint costs, whether it is allowed to spend, whether the promised result was delivered, or how to retain evidence of the exchange. Traditional checkout flows, API-key onboarding and monthly subscriptions were designed for humans, not software acting within a limited mandate.</p>
<h2>What CAPI2 built</h2><p>CAPI2 exposes services through OpenAPI, agent manifests, llms.txt and x402 discovery. A buyer can inspect the price and terms, receive an HTTP 402 payment requirement, settle in USDC on Base and retry the same request with proof. The response is delivered inline as structured JSON. CAPI2 adds preflight policy checks, claim verification, endpoint health, domain-security observations, manifest checks, receipt validation and a combined risk report.</p><div class="flow">discover → quote → policy check → x402 payment → execute → verify delivery → retain receipt</div>
<h2>Where AI is central</h2><p>The customer is often an AI agent. The interfaces are deliberately machine-readable so ChatGPT-compatible tools, MCP clients and other autonomous systems can decide when a service is useful and consume the result without a human-operated dashboard. CAPI2 keeps evidence extraction and payment controls deterministic where possible: an agent can reason with the result, while the commerce layer does not ask a language model to invent whether payment or delivery occurred.</p>
<h2>What makes it different</h2><p>CAPI2 is not a token scheme and it does not promise investment returns. It sells small, concrete digital outcomes. Spending authority can be capped, risky contract terms remain blocked, tests are separated from organic revenue, and a payment can never be presented as a customer sale without independent evidence. That distinction matters for autonomous systems, where a confident status message is not the same as an auditable event.</p>
<h2>Current proof—and the honest limitation</h2><p>The production service publishes twelve paid resources and has completed verified x402 canary transactions that prove the payment-to-delivery loop. A persistent revenue worker now records sequential runs in Postgres and monitors production, discovery, buyer intent and settlements. At publication time CAPI2 has no confirmed organic customer revenue. Canary and owner-funded tests are explicitly excluded. The next milestone is therefore not another feature count; it is a first independently verified buyer.</p>
<h2>What comes next</h2><p>CAPI2 is looking for AI-agent builders, API operators and design partners who need safer pay-per-use commerce. The immediate offer is a free public endpoint preview followed by a $29 Website &amp; API Risk Report. Findings arrive as JSON plus Markdown, with prioritized actions covering security headers, TLS, machine discovery and x402 readiness.</p><a class="cta" href="/buy">Test a public website or API →</a>
<footer class="note">Evidence current at page load. CAPI2 provides public-source observations, not certification, legal advice or guaranteed commercial results.</footer></main></body></html>"""
...
async def sitemap():
    xml = f"""<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"><url><loc>{PUBLIC_ORIGIN}/buy</loc></url><url><loc>{PUBLIC_ORIGIN}/story</loc></url><url><loc>{PUBLIC_ORIGIN}/v1/buyer-catalog</loc></url><url><loc>{PUBLIC_ORIGIN}/.well-known/x402</loc></url><url><loc>{PUBLIC_ORIGIN}/openapi.json</loc></url></urlset>"""
...
def commerce_assurance(payload: CommerceAssuranceRequest):
    ...
    grade = "A" if combined_score >= 85 else "B" if combined_score >= 70 else "C" if combined_score >= 55 else "D"
    actions = [*[f"Publish {name}" for name in missing], *[f"Add {name} header" for name in header_gaps]][:8]
    delivered_at = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
    report_markdown = "\n".join([
        "# Website & API Risk Report", "", f"Target: {target}", f"Grade: {grade}",
        f"Combined score: {combined_score}/100", f"Generated: {delivered_at}", "",
        "## Priority actions", *([f"- {item}" for item in actions] or ["- No priority gaps detected in this point-in-time check."]), "",
        "## Scope and limitations", "- Public, point-in-time external observation", "- No penetration test",
        "- No independent on-chain settlement verification",
    ])
    return {"protocol": "capi2.commerce_assurance/1.1", "report_id": f"car_{uuid.uuid4().hex[:16]}", "organization": payload.organization, "target_url": target, "combined_score": combined_score, "grade": grade, "discovery": discovery, "domain_security": security, "priority_actions": actions, "report_markdown": report_markdown, "limitations": ["Point-in-time external observation", "No penetration test", "No independent on-chain settlement verification"], "delivered_at": delivered_at}
